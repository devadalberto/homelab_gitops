...
5. **Download pfSense installation media.**
   ...
   - If the downloaded archive does not already match `netgate-installer-amd64-serial.img.gz`, either rename it or set
     `PF_INSTALLER_SRC` in `.env` to the exact filename you downloaded. The automation accepts the `.img.gz` archive as is
     (decompression to `.img` is optional—update the path if you extract it).
   - Legacy `PF_SERIAL_INSTALLER_PATH`/`PF_ISO_PATH` knobs remain supported when migrating older environments. Define `PF_ISO_PATH`
     and supply `--no-headless` when invoking `pf-bootstrap.sh` if you intentionally bootstrap the VGA build.
...
6. **Create and populate `.env`.**
   ...
   - Set `PF_INSTALLER_SRC` to the absolute path of the downloaded pfSense installer so the bootstrap scripts can validate and
     mount it. Legacy `PF_SERIAL_INSTALLER_PATH`/`PF_ISO_PATH` values are still honored when present. If you customize the pfSense
     LAN bridge name, update both `PF_LAN_BRIDGE` and `PF_LAN_LINK` to reference the same value (defaults to `pfsense-lan`).
...
8. **Define the pfSense VM before cluster bring-up.**
   - Execute `./pfsense/pf-bootstrap.sh --installation-path "$PF_INSTALLER_SRC"` to create the libvirt domain, attach the
...
1. **Prepare the host operating system**
  ...
  - Populate `.env` with your domain, LAN addressing, MetalLB pool, and pfSense installer details. Critical variables consumed
    by downstream scripts include `PF_INSTALLER_SRC` (legacy `PF_SERIAL_INSTALLER_PATH`/`PF_ISO_PATH` are still honored, with the
    latter used for VGA installs), `PF_HEADLESS`, `WORK_ROOT`, `VM_NAME`, `DISK_SIZE_GB`, `RAM_MB`, `VCPUS`, `WAN_MODE`, and
    `WAN_NIC`. If your downloaded installer uses a different filename, either rename it to `netgate-installer-amd64-serial.img.gz`
    or set `PF_INSTALLER_SRC` directly. You may leave the `.img.gz` archive compressed (or extract it to `.img` and update the
    path accordingly).
...
```bash
./pfsense/pf-bootstrap.sh --installation-path "$PF_INSTALLER_SRC"
virsh start "${VM_NAME}"
virsh console "${VM_NAME}"
```
  - `PF_INSTALLER_SRC` should reference the pfSense installer you downloaded. Rename the archive to
    `netgate-installer-amd64-serial.img.gz` or update `.env` with the actual filename (compressed `.img.gz` and `.iso.gz`
    archives work without manual extraction, though you can extract to `.img` if preferred). Legacy
    `PF_SERIAL_INSTALLER_PATH`/`PF_ISO_PATH` values continue to work for existing setups.
...
1. **Clone the repository and prepare environment variables**
  ...
  `PF_INSTALLER_SRC` points at the pfSense installer you downloaded (legacy `PF_SERIAL_INSTALLER_PATH`/`PF_ISO_PATH` still work) and adjust `PF_HEADLESS` if you prefer a VNC console (`--no-headless`).
